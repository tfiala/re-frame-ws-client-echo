...
* [`src/chat_re_frame/`](src/chat_re_frame/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/chat_re_frame/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `chat-re-frame` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/chat_re_frame/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns chat-re-frame.example
  (:require [chat-re-frame.config :as config])
